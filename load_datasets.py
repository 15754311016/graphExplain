...
POLARITY_PATH = 'E:\LIME\lime-experiments-master\processed_acl/'
...
def LoadMultiDomainDataset(path_data, remove_bigrams=True):
  ...
  def get_words(line, remove_bigrams=True):
    ...
    return z
  for line in open(os.path.join(path_data, 'negative.review'),encoding='utf-8'):
    neg.append(get_words(line, remove_bigrams))
  for line in open(os.path.join(path_data, 'positive.review'),encoding='utf-8'):
  ...
  # print(type(test_data), len(test_labels))
