...
def nlars(X, X_ty, num_feat, max_neighbors):
    ...
    while sum(c[A]) / len(A) >= 1e-9 and len(A) < num_feat + 1:
        ...
        # print("X:", X,X.shape)
        # print("A:", A)
        # print("DOT:", np.dot(X[:, A].transpose(), X[:, A]))
        # print("DOT:", np.linalg.det(np.dot(X[:, A].transpose(), X[:, A])))
        # print("s:",s)
        w = np.linalg.lstsq(np.dot(X[:, A].transpose(), X[:, A]), s)[0]
        XtXw = np.dot(X.transpose(), np.dot(X[:, A], w))

        gamma1 = (C - c[I]) / (XtXw[A[0]] - XtXw[I])
        # print("(XtXw[A[0]] - XtXw[I]:",(XtXw[A[0]] - XtXw[I]))
        # print("gama1:",gamma1)
        gamma2 = -beta[A] / (w)
        gamma3 = np.zeros((1, 1))
        ...
        XtXbeta = np.dot(X.transpose(), np.dot(X, beta))
        # print("xtxbata", XtXbeta.shape)
        c = X_ty - XtXbeta
        # print("c[I]:",c[I],c[I].shape)
        j = np.argmax(c[I])
        C = max(c[I])
        # print("!!!!!")
        k += 1
    ...
    # print("path_final:",path_final.shape)
    # print("lam_final:",lam_final.shape)
